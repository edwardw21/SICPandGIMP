...
;; Check if procedure available in available-procedures (in imageops)

(define (module-procedure? expression)
  (let ((procedure-entry (assoc (operator expression) available-procedures)))
    procedure-entry))

(define-generic-procedure-handler g:eval
  (match-args module-procedure? environment?)
  (lambda (expression environment)
    ;; load module
    (load (string-append
	   "modules/"
	   (cadr (module-procedure? expression))))
    (g:apply (operator expression)
	     (operands expression)
	     environment)))
